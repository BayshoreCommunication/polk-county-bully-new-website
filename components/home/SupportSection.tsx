...
export default function SupportSection() {
  return (
    ...
    bg-cover 
    bg-center 
    bg-no-repeat py-12 px-6" // Reduced padding
      style={{ backgroundImage: "url('/images/homepage/support/bg.webp')" }}
    >
      <div className="flex flex-col justify-center items-center text-center">
        <Reveal y={-100} duration={2}>
          <h2 className="text-3xl md:text-6xl mb-3 text-center text-stroke-white drop-shadow-[6px_6px_0px_#000]"> {/* Reduced text size and margin */}
        ...
        <Reveal y={-100} duration={3}>
          <p className="text-sm md:text-base opacity-90 mb-8 text-center w-full max-w-2xl text-white"> {/* Reduced margin */}
      ...
      <div className="container mx-auto flex flex-col lg:flex-row items-center justify-between gap-8"> {/* Reduced gap */}
        {/* LEFT CONTENT */}
        <div className="text-white max-w-xl">
          <div className="max-w-xl">
            <Reveal y={-100} duration={2}>
              <h3 className="text-3xl md:text-5xl font-extrabold mb-3 text-stroke-black drop-shadow-[6px_6px_0px_#000]"> {/* Reduced text size and margin */}
                ADOPT
              </h3>
            </Reveal>

            <Reveal y={-100} duration={3}>
              <p className="text-white text-opacity-90 mb-4 text-sm md:text-base"> {/* Reduced margin */}
            ...
            <Reveal y={100} x={100} duration={2}>
              <a href="https://petstablished.com/adoptions/personal-information?application_type=Adopt&donation_section=false&form_id=24575&form_type=generic&generic_form_id=24575&pet_id=715879&section=1&selected_pets=false">
                <button className="bg-white text-primary font-semibold px-5 py-2 rounded-full shadow hover:bg-gray-100 transition"> {/* Reduced padding */}
        ...
        {/* RIGHT IMAGE */}
        <div className="relative w-full max-w-lg"> {/* Reduced max-width */}
      ...
      <div className="absolute bottom-0 w-full left-0 right-0 h-12 bg-gradient-to-b from-transparent to-blue-100 "></div> {/* Reduced height */}
    </section>
  );
}